                                ...
                                <ul class="messages">
                                  <li>
                                    <div class="message_wrapper">
                                      ...
                                      <?php } ?>
                                      <button onclick="bitacoraEstatus(<?=$empresa->id?>)"  type="button" class="btn btn-success btn-sm" >Bitacora</button>
                                    </div>
                                  </li>
                                </ul>
                              </div>
                                  ...
                                  <tr <?= $color ?>>
                                    ...
                                    <?php if ($this->session->privilegios['administrar_empresas']) { ?>
                                      <td>
                                        ...
                                        <button onclick="bitacoraContactos(<?= $elem->id ?>)" class="btn btn-success btn-xs" value="<?= $elem->id ?>"><i class="fa fa-book"></i> Bitacora</button>
                                      </td>
                                    <?php } ?>
                            ...
                            <div class="row">
                              ...
                              <div class="col-md-12 col-sm-12 col-xs-12">   
                                ...
                                <div class="row">
                                  <p>
                                    Persona Fisica
                                    <input type="radio" class="flat prov" id="cbFisica" name="cbPersona" value="FISICA" <?php if($proveedor->persona == 'FISICA') { echo 'checked'; }?>/>
                                    Persona Moral
                                    <input type="radio" class="flat prov" id="cbMoral" name="cbPersona" value="MORAL" <?php if($proveedor->persona == 'MORAL') { echo 'checked'; }?>/>
                                  </p>
                                </div>
                                <div class="row">
                                ...
                                <div class="row">
                                  <div class="col-md-4 col-sm-4 col-xs-12">
                                    ...
                                    <button onclick="bitacoraEstatus(<?= $empresa->id?>);"  type="button"  class="btn btn-success"><i class="fa fa-book"></i> Bitacora</button>
        ...
        <div class="modal fade empresa" role="dialog" aria-hidden="true">
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              <form method="POST" action=<?= base_url('empresas/editar') ?> class="form-horizontal form-label-left" >
             ...
             <!-- <div class="item form-group">
              ...
              <div class="item form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                      ...
                      ?>
                      <p>Prospecto
                        <input type="checkbox"  name="prospecto" id="prospecto" class="flat" value="1" <?php if($empresa->prospecto) { echo 'checked'; }?> /> 
                      </p>
                      <?php
                    }
                    ?>
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <p style="display: inline; font-size: 15px">
                  ...
                  <select style="display: inline; margin-right: 10px;" required="required" class="selectpicker" id="documento" name="documento">
                    <option value=''></option>
                    <?php foreach ($documento as $elem) { ?>
                    <option value=<?= $elem->id ?>><?= ($elem->tipo) ?></option>
                    <?php } ?>
                  </select>
                </div>
              <form>
...
<div id="mdlBitacora" class="modal fade bs-example-modal-md" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    function eventos(){
      ...
      $("#cbResico").on('ifChanged', function (e) {
        resico();
      });
...
function initMap() {
      ...
      $.ajax({
          ...
          success: function(result){
            if(result){
              ...
              ren.insertCell(6).innerHTML = "<button onclick='getContacto(this)' class='btn btn-warning btn-xs' value='" + res.id + "'><i class='fa fa-pencil'></i> Editar</button> <button onclick='deleteContacto(this)' value='" + res.id + "' class='btn btn-danger btn-xs'><i class='fa fa-trash'></i> Eliminar</button> <button onclick='bitacoraContactos('" + res.id + "')' class='btn btn-success btn-xs' value='" + res.id + "'><i class='fa fa-book'></i> Bitacora</button>";
    ...
    var currentFileRow; var currentFileName; var currentFileImage; var currentTipo; var currentClave;
    ...
    alert('Seleccione tipo de archivo');
      ...
      var documento = _("documento");
      insertFileRow(file.name, comm.value, documento.value);
    ...
    function completeHandler(event){
      ...
      currentTipo= "<a>" + res.tipo + "</a>";
      currentClave= "<a>" + res.clave + "</a>";
    ...
    function editArchivo(){
      ...
      var documento = _("documento").value;
      var tab = $('#tablaArchivos')[0];
      ...
      $.ajax({
          ...
          data: { 'id' : idFile, 'comentarios' : comm, 'documento' : documento, },
          success: function(result){
            if(result) {
    ...
    function guardarProveedor(){
      ...
      var valResico= $('#txtResico').val();
      var persona = $("input[name='cbPersona']:checked").val();
      //alert(persona);
      ...
      */
      ...
      $.ajax({
          ...
          data: { 'id_empresa' : '<?= $empresa->id ?>', 'tipo' : JSON.stringify(tipo), 'clasificacion_proveedor' : clasificacion_proveedor, 'formas_pago' : JSON.stringify(formas_pago), 'credito' : credito, 'rma_requerido' : rma_requerido, 'monto_credito' : montoCredito, 'terminos_pago' : terminosPago, 'formas_compra' : JSON.stringify(formas_compra), 'moneda_credito' : monedaCredito, 'entrega' : entrega, 'aprobado' : aprobado, 'tags' : tags, pasos_cotizacion : JSON.stringify(pasos_cotizacion), pasos_compra : JSON.stringify(pasos_compra), 'valResico' : valResico, 'persona' : persona },
          success: function(result) {
...
<script type="text/javascript">   
    $(document).ready(function() {                       
        $("#pais").change(function() {
                ...
                pais = $('#pais').val();
                $.post(base_url+"empresas/estados", {
                paisid : pais
                }, function(data) {
                    $("#estado").html(data);
                });
