...
function buscar(){
    ...
    $.ajax({
        type: "POST",
        url: URL,
        data: { prioridad: JSON.stringify(prioridad), estatus: estatus, texto : texto, parametro : parametro, misprs : misPRs,fecha1 : fecha1, fecha2 : fecha2 },
		...
		$('#lblCount').text(rs.length + (rs.length == 1 ? " Pr" : " Pr's"));
                $.each(rs, function(i, elem){
                    ...
                    if(elem.estatus == 'EN PO' | elem.estatus == 'PO AUTORIZADA' | elem.estatus == 'CERRADO')
                    {
                        ren.insertCell().innerHTML = "<button class='btn btn-default btn-xs'><i class='fa fa-shopping-cart'></i> PO: "+ elem.POActual +"</a>";
                    }
                    else{
