...
function buscar(){
    ...
    $.ajax({
        type: "POST",
        url: URL,
        data: { prioridad: JSON.stringify(prioridad), tipo: JSON.stringify(tipo), estatus: estatus, texto : texto, parametro : parametro, usuario : user, asignado : asignado, archivo : archivo, fecha1 : fecha1, fecha2 : fecha2 },
...
document.getElementById("fileEx").style.visibility = "hidden";
            ...
            if(result)
            {
                ...
                $.each(rs, function(i, elem) {
                    ...
                    if(elem.nominado == 1)
                    {
                        ...
                        if (elem.nombre_archivoEjemplo) {
                            document.getElementById("fileEx").style.visibility = "visible";
                            ren.insertCell(8).innerHTML = "<a target='_blank' href='" + base_url + 'compras/getFileExample/' + elem.QR + "'><img height='25px' src='" + file_image(elem.nombre_archivo) + "'></a>";
        ...
        $.ajax({
            ...
            success: function(result) {
                if(result)
                {
                    ...
                    var answer = confirm('Item asignado a PR: '+pr+' ¿Estas seguro de usarlo?');
                    if(answer)
                    {
                       buscarML();
                    }            
...
function buscarMLproducto(){
   ...
   // alert(item);
        $.ajax({
            type: "POST",
            url: URL,
            data: { item: item, destino:DESTINO, tipo:CURRENT_TIPO },
            success: function(result) {
                if(result)
                {
                    ATT = JSON.parse(result);
                    ...
                    $.each(ATT, function(i, elem) {
                        if (DESTINO !=  'VENTA' && CURRENT_TIPO == 'PRODUCTO') {
                            alert("AQUI");
                            agregarAtributos();
                        }
...
function cargarItems(){
    var URL = base_url + "/compras/cargarItems";
    var item = $('#txtItem').val();
    var qty = $('#txtQtySolicitud').val();
    var tqty =$('#tblAtributos tbody tr').length;
...
function consumoInterno(){
    var URL = base_url + "compras/checkDestino";
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (CURRENT_TIPO == "PRODUCTO" && DESTINO != 'CONSUMO INTERNO') {
                    alert(CURRENT_TIPO + ' '+ DESTINO);
                    alert(1);
                    $('#mdlDescripcionQR').modal();
                    $('#idItems').hide();
                    $('#serieItems').hide();
                ...
                else if(CURRENT_SUBTIPO == "CALIBRACION" || CURRENT_SUBTIPO == "REVISION" || CURRENT_SUBTIPO == "REPARACION" || CURRENT_SUBTIPO == "MANTENIMIENTO")
                {
                    alert(2);
                    
                    if (DESTINO == 'CONSUMO INTERNO') {  
                        alert(3);
                        $('#mdlConsumoInterno').modal();
                        $("#mdlTituloQR").html('QR de CONSUMO INTERNO');
                    }else{
                        alert(4);
                    $('#mdlDescripcionQR').modal();
                    cargarItems();    
