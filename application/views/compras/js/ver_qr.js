...
function proveedoresAsignados(){
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            const label = document.getElementById('vigencia');
            const dateObj = new Date(vencimiento); // Convertir a objeto Date
            const formattedDate = dateObj.toISOString().split('T')[0];

            if(hayProv && moment(vencimiento) > moment(new Date()))
            {
                label.innerText = "Vigencia: "+formattedDate; // Cambia el texto del label
...
function bitacoraEstatus(id){
    ...
    var URL = base_url + "compras/bitacoraQR";
    var dic = {}
    //alert(CURRENT_PR);
    $.ajax({
        type: "POST",
