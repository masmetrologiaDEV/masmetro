...
function buscar(){
    ...
    $.ajax({
        type: "POST",
        url: URL,
        data: { prioridad: JSON.stringify(prioridad), estatus: estatus, texto : texto, parametro : parametro },
		...
		$('#lblCount').text(rs.length + (rs.length == 1 ? " PO" : " PO's"));
                ...
                $.each(rs, function(i, elem){
                    ...
                    ren.insertCell(3).innerHTML = elem.Prov;
                    ren.insertCell(4).innerHTML = elem.Contact;
                    ren.insertCell(5).innerHTML = elem.entrega;
                    
                    var btn = ""; var link = base_url + "ordenes_compra/ver_po/"+ elem.id;
                    switch (elem.estatus)
                    {
                        case 'EN PROCESO':
                    ...
                    ren.insertCell(6).innerHTML = "<a target='_blank' href='" + link +"' class='" + btn + "'>" + elem.estatus + "</a>";
                });
            }
            else
            {
