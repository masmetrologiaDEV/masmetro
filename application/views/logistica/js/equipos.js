...
function buscar(){
    ...
    var opc = $("input[name=rbBusqueda]:checked").val();
        ...
        success: function(result) {
            if(result)
            {
                ...
                $.each(rs, function(i, elem){
                    ...
                    //ren.insertCell().innerHTML = agregarLoteTemp(elem.id, elem.estatus_factura, elem.usuario);
                    ren.insertCell().innerHTML = "<button type='button' onclick='agregarLoteTemp("+ITEMS+");' class='btn btn-warning btn-xs'><i class='fa fa-plus'></i> Agregar</button>";
                });
            }
            else
            {
...
function agregarLoteTemp(it){
    ...
    var URL = base_url + "logistica/agregarLoteTemp";
    item=it;
    $.ajax({
        type: "POST",
        url: URL,
...
function CargarLoteTemp(){
    ...
    var URL = base_url + "logistica/cargarLoteTemp";
        ...
        success: function(result) {
            if(result)
            {
                ...
                $.each(r, function(i, elem){
                    var ren = tab.insertRow(tab.rows.length);
                    ren.insertCell().innerHTML = elem.rs;
                    ren.insertCell().innerHTML = elem.items;
