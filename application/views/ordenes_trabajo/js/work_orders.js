...
var dateTime = date + ' ' + time;
 ...
 //alert(fecha +' < '+dateTime);
if (fecha < dateTime) {
    alert("Fecha Programada debe ser mayor a la actual");
   // alert(Date.parse(fecha)+" < "+Date.parse(dateTime))
    return false;
}
    if ($('#tblConceptosRS tr').length ==1) {
    alert("Seleccione los ítems a incluir en la orden de trabajo.");
        return false;
}
...
function enviarSolicitud(){
    if(validacion())
    {
        ...
        var rs=null;
        wo.FProgramado =$('#txtFechaAccion').val().trim();
       // wo.Comentarios =$('#txtComentarios').val().trim();
...
var formdata = new FormData();
        ...
        formdata.append("wo", JSON.stringify(wo));
...
function complete(evt){
    ...
    window.location.href = base_url + 'ordenes_trabajo/';
...
function mdlReprogramar(id){
    $('#txtComentarios').val("");
    $('#itemR').val(id);
    $('#btnComentario').show();
    $('#btnRechazar').hide();
    $('#mdlComentarioTitle').text('Agregar Comentario');
    $('#mdlReprogramar').modal();
}
function mdlRealizado(id){
    $('#txtComentarios').val("");
    $('#itemOk').val(id);
    $('#btnComentario').show();
    $('#btnRechazar').hide();
...
function mdlCancelarWO(id){
    $('#txtComentarios').val("");
    $('#itemCancelarWO').val(id);
    $('#btnComentario').show();
    $('#btnRechazar').hide();
    $('#mdlCancelarwo').modal();
}
function mdlCerrarWO(id){
    $('#txtComentarios').val("");
    $('#itemCerrar').val(id);
    $('#btnComentario').show();
    $('#btnRechazar').hide();
...
function conteoTablaRS(){
    ...
    $('#lblItemsCount').html("<b>" + c + " Items</b>");
}
