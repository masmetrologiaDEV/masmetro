...
class Logistica extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('logistica_model','Modelo');
    ...
    function lotes(){
        $data['lote']=$this->Modelo->lotes();
        //echo var_dump($data);die();
        $this->load->view('header');
        $this->load->view('logistica/lotes', $data);
    }
    function lotes_conceptos(){
        $this->load->view('header');
        $this->load->view('logistica/lotes_conceptos');
    }
    ...
    function ajax_getMLEquipos(){
        ...
        $query="SELECT *, ifnull(Item,'') as Equipo_ID, ifnull(fabricante,'') as Fabricante, ifnull(modelo,'') as Modelo, ifnull(serie,'') as Serie from v_items_entregando";

        
        if($opc=='rs'){
            $query .=" where RS = '".$txt."'";
        }elseif($opc=='cliente'){
            $query .=" where NombreCorto like '%".$txt."%'";
        }
    ...
    function agregarLoteTemp()
    {
        ...
        $query="SELECT RS, Item, Descripcion, fabricante, modelo, serie from v_items_entregando where Item='".$item."'";
        ...
        foreach($res as $elem){
            $RS=$elem->RS;
            $items=$elem->Item;
            $desc=$elem->Descripcion." ".$elem->fabricante." ".$elem->modelo." Serie: ".$elem->serie." ID: ".$elem->Item;
        }
        ...
        echo $this->Modelo->insertarLoteTemp($datos);
    ...
    function registrarLote(){
        ...
        $this->Modelo->registrarLote($datos);
        $query="SELECT * FROM lotes WHERE id=(SELECT MAX(id) FROM lotes) and idUs=".$this->session->id;
       ...
       // $path= fopen($p,"rb");
        ...
        $t='https://validacion.masmetrologia.mx/gedmas/inicio/lote/'.$idLote;
        $qr='Lote_'.$idLote.".png";
        $file = 'data/logistica/qr/'.$qr;
        QRcode::png($t, $file, 'L', 10, 2);
        $this->Modelo->QR($idLote,array('qr'=>$qr));
        //die();
        $queryTemp="SELECT * FROM lotes_conceptosTemp WHERE idus=".$this->session->id;

        $resTemp = $this->Conexion->consultar($queryTemp);
        ...
        $this->Modelo->registrarLoteConceptos($data);
        }
        
        $this->Modelo->deleteLoteTemp();
        redirect(base_url('logistica/ver_lote_pdf/'.$idLote));
 ...
 function ver_lote_pdf($id){
        $query="SELECT l.*,l.id as IdLote, lc.* FROM lotes l JOIN lotes_conceptos lc on l.id=lc.idlote WHERE l.id='$id'";
        $res = $this->Conexion->consultar($query);
        $query2="SELECT l.*,l.id as IdLote, lc.*, concat(u.nombre, ' ', u.paterno ) as UsEntrega FROM lotes l JOIN lotes_conceptos lc on l.id=lc.idlote LEFT JOIN usuarios u on u.id= l.usEntrega WHERE l.id='$id'";
        $result = $this->Conexion->consultar($query2, TRUE);
        $qr =  '<img height="100" width="100" src="'. base_url('data/logistica/qr/'.$result->qr).'"/>';
        
        $firma =  '<img height="100" width="100" src="data:image/png;base64,'.base64_encode($result->firma).'"/>';
        $loc ='<a target="_blank" id="enlace" href="'.$result->locazion.'">Abrir en Google Maps</a>';
        ...
        $FECHA = date_format(date_create($result->fecha), 'd/m/Y h:i A');
        ...
        $pdf->SetHeaderData(PDF_HEADER_LOGO_ORIGINAL, '40','                                                        Lote :' . $lote, "                                                           Fecha: " . $FECHA );
        ...
        $pdf->SetFont('times', 'B', 15);
        $pdf->MultiCell(90, 0, "Reporte de entrega de equipos", 0, 'L', 1, 0, '', '', true, 0, false, true, 0);
        $pdf->Ln();
        ...
        if ($result->UsEntrega) {
            ...
            $pdf->MultiCell(90, 0, "Entregado por: ".$result->UsEntrega, 0, 'L', 1, 0, '', '', true, 0, false, true, 0);
            
          } 
        $pdf->MultiCell(1, 0,   "", 0, 'L', 1, 1);
        $pdf->writeHTML($qr, 180, 60, 15, 15,'R');        
        ...
        $pdf->SetTextColor(0);
        $pdf->Cell(15, 6, "RS", 1, 0, 'C', 0);
        $pdf->Cell(25, 6, "Items", 1, 0, 'C', 0);
        $pdf->Cell(115, 6, "Descripción", 1, 0, 'C', 0);
        $pdf->Cell(25, 6, "Estatus", 1, 0, 'C', 0);
        $pdf->Ln();

        $w = array(15, 25 , 115, 25);
        $pdf->SetFont('', '', 10);
        ...
        foreach($res as $elem) {
            ...
            $cellcount[] = $pdf->MultiCell($w[0], 6, $elem->rs, 0, 'C', 0, 0);
            
            $cellcount[] = $pdf->MultiCell($w[1], 6, $elem->items, 0, 'L', 0, 0);   
            
            
            
            $cellcount[] = $pdf->MultiCell($w[2], 6, $elem->descripcion, 0, 'R', 0, 0);
            $cellcount[] = $pdf->MultiCell($w[3], 6, $elem->estatus, 0, 'R', 0, 0); // VER AQUI
         ...
         for ($startY; $startY < 150; $startY = $startY + 6) { 
            $pdf->MultiCell($w[0], 6, '---', 'LR', 'C', 0, 0);            
            $pdf->MultiCell($w[1], 6, '   ---------------   ', 'LR', 'R', 0, 0);
            $pdf->MultiCell($w[2], 6, '   -------------------------------------------------------------------------------------------', 'LR', 'L', 0, 0);
            $pdf->MultiCell($w[3], 6, '   ---------------   ', 'LR', 'R', 0, 0);
            $pdf->Ln();
        }

        $pdf->MultiCell(0, 6, '', 'T', 'C', 0 , 0, '', '', TRUE, 0, FALSE, TRUE, 0, 'M');
        $pdf->Ln();
        
        
        if (!is_null($result->firma) ) {
        $pdf->writeHTML($firma, 180, 60, 15, 15,'L');
        $pdf->setFillColor(255,255,255); 
        $pdf->SetFont('times', 'B', 12);
        $pdf->MultiCell(150, 8,"Firma de recibido", 0, 'L', 1, 0, '', '', true, 0, false, true, 0);
        }
        if (!is_null($result->localizacion) ) {
            $pdf->Ln();
            $pdf->Write(10, 'Abrir ubicacion en Maps', $result->localizacion, false, 'L', true);
    ...
    function ver_lote($id)
    {
        $data['lote']=$this->Modelo->lote($id);
        $data['lotes']=$this->Modelo->lotes_conceptos($id);
        //echo var_dump($data);die();
        $this->load->view('header');
        $this->load->view('logistica/ver_lote', $data);
    }
