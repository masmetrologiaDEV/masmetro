...
class ordenes_trabajo extends CI_Controller {
    ...
    function calendario(){
        $data['users'] = $this->Conexion->consultar("SELECT U.id, concat(U.nombre, ' ', U.paterno) as Nombre,U.color, P.puesto as Puesto, U.correo from usuarios U inner join puestos P on U.puesto = P.id where U.activo = 1 and U.departamento='OPERACIONES'");
        $data['autos'] = $this->Conexion->consultar("SELECT id, concat(fabricante, '  ', placas) as name from autos");
        $data['patrones'] = $this->Conexion->consultar('SELECT * from patrones');

        $this->load->view('header');
        $this->load->view('ordenes_trabajo/calendario', $data);
    }
...
$pdf->Output(sys_get_temp_dir().'/'.$name, 'I');
    ...
    function ajax_getUsuarios(){
        $query = "SELECT U.id, concat(U.nombre, ' ', U.paterno) as Nombre, P.puesto as Puesto, U.correo from usuarios U inner join puestos P on U.puesto = P.id where U.activo = 1 and U.departamento='OPERACIONES'";
    ...
    function ajax_getPatrones(){
        $query = "SELECT * from patrones";
    ...
    function ajax_getAutos(){
        $query = "SELECT id, marca, placas from autos";
    ...
    function validarRecurso()
    {
        $tipo=$this->input->post('tipo');
        $id=$this->input->post('id');
        $query="SELECT * from evento_temp where id_recurso=$id and tipo='$tipo'";
        //echo $query;die();
    ...
    function AsignarRecurso()
    {
        $datos  = array(
            ...
            'id_recurso' => $this->input->post('id'), 
            'tipo' => $this->input->post('tipo'), 
        );

        $res = $this->Conexion->insertar('evento_temp', $datos);
        if ($res) {
            echo 1;
        }
    }
    function get_tecnicos(){
        $query = "SELECT et.*, concat(u.nombre, ' ', u.paterno) as name FROM evento_temp et JOIN usuarios u on u.id=et.id_recurso WHERE tipo = 'us' and et.idus = ".$this->session->id;
    ...
    function get_patrones(){
        $query = "SELECT et.*, p.nombre as name FROM evento_temp et JOIN patrones p on p.id=et.id_recurso WHERE et.tipo = 'pt' and et.idus= ".$this->session->id;
    ...
    function get_autos(){
        $query = "SELECT et.*, concat(a.fabricante, ' - ', a.placas) as name FROM evento_temp et JOIN autos a on a.id=et.id_recurso WHERE et.tipo = 'at' and et.idus= ".$this->session->id;
    ...
    function eliminar_tecnico()
    {
        ...
        if ($id) {
            $where['id'] =$id;
            $deleted = $this->Conexion->eliminar('evento_temp', $where);
    ...
    function validacion()
    {
        ...
        $res = $this->Conexion->consultar("SELECT * FROM `eventos` WHERE inicia <= '".$termina."' AND termina >= '".$inicia."'");
      if($res)
      {
        echo json_encode($res);
      }    
    ...
    function crearEvento()
    {
       $datos = array(
            ...
            'repetir' => $this->input->post('repeatEvent'),
        );
        $id_evento = $this->Conexion->insertar('eventos', $datos);

        $res = $this->Conexion->consultar('SELECT * from evento_temp where idus='.$this->session->id);
        foreach ($res as $key) {
            $data = array(
                'id_evento' => $id_evento,
                'idus' => $key->idus, 
                'id_recurso' => $key->id_recurso,
                'tipo' => $key->tipo,
            );
        $this->Conexion->insertar('evento_recursos', $data);
            
        }
        $where['idus'] =$this->session->id;
        $this->Conexion->eliminar('evento_temp', $where);
            
    }
    function getEventos() {
    $selectUsuario = $this->input->post('selectUsuario'); // Usar POST, no GET
    $selectAuto = $this->input->post('selectAuto');
    $selectPatron = $this->input->post('selectPatron');

    $query = 'SELECT DISTINCT A.id as id_evento, A.fecha, A.inicia as start, A.termina as end, A.titulo as title, A.usuario, concat(U.nombre, " ", U.paterno) as user, A.quien, A.ubicacion, U.color, er.id_recurso 
              FROM eventos A 
              JOIN evento_recursos er ON A.id = er.id_evento 
              JOIN usuarios U ON er.id_recurso = U.id 
              WHERE 1=1';
    
    if ($selectUsuario && $selectUsuario != 'TODO') { // Validamos que no sea 'TODO'
        $query .= ' AND tipo="us" AND er.id_recurso = ' . $selectUsuario;
    }
     if ($selectAuto && $selectAuto != 'TODO') { // Validamos que no sea 'TODO'
        $query .= ' AND er.id_recurso = ' . $selectAuto;
    }
     if ($selectPatron && $selectPatron != 'TODO') { // Validamos que no sea 'TODO'
        $query .= ' AND er.id_recurso = ' . $selectPatron;
    }
   // echo $query;die();
    $data = $this->Conexion->consultar($query);
    echo json_encode($data);
}

    function get_tecnicos_eventos(){
        $id=$this->input->post('id');

        $query = "SELECT et.*, concat(u.nombre, ' ', u.paterno) as name FROM evento_recursos et JOIN usuarios u on u.id=et.id_recurso WHERE tipo = 'us' and et.id_evento = ".$id;
//echo $query;die();
        $res = $this->Conexion->consultar($query);
        if($res)
        {
    ...
    function get_patrones_eventos(){
        $id=$this->input->post('id');
        $query = "SELECT et.*, p.nombre as name FROM evento_recursos et JOIN patrones p on p.id=et.id_recurso WHERE et.tipo = 'pt' and  et.id_evento = ".$id;
    ...
    function get_autos_eventos(){
        $id=$this->input->post('id');
        $query = "SELECT et.*, concat(a.fabricante, ' - ', a.placas) as name FROM evento_recursos et JOIN autos a on a.id=et.id_recurso WHERE et.tipo = 'at' and et.id_evento = ".$id;
