...
class Facturas extends CI_Controller {
    ...
    function ajax_setSolicitud(){
       ...
       // if(isset($_FILES['other'])){   
          ...
          //  echo var_dump($solicitud->correos);die();
...
//echo $query;die();
    ...
    function ajax_getReporteServicios(){
        ...
        $query = "SELECT item_id, folio_id, descripcion,rsn.Notas, concat(descripcion, if(isnull(Fabricante), '', concat(' ', Fabricante)), if(isnull(Modelo), '', concat(' ', Modelo)), if(isnull(Serie), '', concat(' Serie: ', Serie)), if(isnull(Equipo_ID), '', concat(' ID: ', Equipo_ID))) as CadenaDescripcion, Solicitud_ID, Monto from rsitems  where 1=1 ";
