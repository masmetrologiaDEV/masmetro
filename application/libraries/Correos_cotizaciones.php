...
class Correos_cotizaciones {
    ...
    function aprobacionCotizacion($datos) {
        ...
        $CI->email->subject('Cotización Masmetrologia - '.$idCompleto);
        $CI->email->message($mensaje);

        $CI->email->send();
    }
    function cancelarSeguimiento($datos) {
        $CI = & get_instance();
        $CI->load->library('email');
        ...
        $mensaje = "
            <img width='400' src='$logo'><br>
            <h1><font face='Times'>SIGA-MAS</font></h1>
            <h2>Se ha cancelado el seguimiento de la cotización</h2>
            <p><b>ID:</b> $idCompleto</p>
            <p><b>Fecha limite:</b> " . $datos['fecha_limite'] . "</p>
            <p><b>Acción:</b> " . $datos['accion'] . "</p>
            <br>";
        ...
        $CI->email->from('tickets@masmetrologia.com', 'SIGA-MAS');
        $CI->email->to($datos['correos'], $datos['correo_cancelar']);

        $CI->email->subject('Cancelacion de seguimiento');
        $CI->email->message($mensaje);
